...
complete_file_name(char ***result_ptr, char **path_prefix, const char *input_path, size_t size)
{
    ...
    *p = 0;
    ...
    cand = (char **)realloc(cand, sizeof(char *) * now);
    count = now;
                         ...
                         JSValueRef* exception)
{
    if (argc == 1 && JSValueIsString(context, argv[0]))
    {
        ...
        JSValueRef *arr = (JSValueRef *)malloc(sizeof(JSValueRef) * csize);
        int i;
...
static JSValueRef
js_cb_complete_prog(JSContextRef context,
                    JSObjectRef function,
                    JSObjectRef self,
                    size_t argc,
                    const JSValueRef argv[],
                    JSValueRef* exception)
{
    if (argc == 1 && JSValueIsString(context, argv[0]))
    {
        ...
        while (dir != NULL)
        {
            ...
            char *temppath;
            size_t tempsize = asprintf(&temppath, "%s/%s", dir, path);
            char **comp;
            size_t csize = complete_file_name(&comp, NULL, temppath, tempsize);

            if (csize > 0)
            {
                int i;
                for (i = 0; i < csize; ++ i)
                    comp_all.push_back(comp[i]);
                free(comp);
            }
        ...
        {
            JSStringRef str = JSStringCreateWithUTF8CString(comp_all[i]);
            arr[i] = JSValueMakeString(context, str);
...
extern "C" {
    ...
    init(weblet_host_interface_t host_interface)
    {
        ...
        host->register_native_method("CompleteProg", js_cb_complete_prog);
